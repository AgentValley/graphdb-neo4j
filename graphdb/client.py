...
class Neo4jClient:
    ...
    def create_concept(self, concept_id, question, answer, course_id, prerequisites=None):
        query = "MATCH (c:Course {course_id: $course_id}) " \
                "MERGE (co:Concept {concept_id: $concept_id, que: $question, ans: $answer}) " \
                "MERGE (c)-[:HAS]->(co) " \
                "RETURN co"
        parameters = {"concept_id": concept_id, "question": question, "answer": answer, "course_id": course_id}
        with self.driver.session() as session:
